...
def gen_imagedata(args):
    ...
    cmd = [sys.executable, IMAGEDATA_SCRIPT] + args
    ...
    if result.returncode != 0:
        raise RuntimeError(
            f"generate_imagedata failed (rc={result.returncode}) for args={args}: {result.stderr.strip()}"
        )

    lines = [line.strip() for line in result.stdout.splitlines() if line.strip()]
    data_uri = next((line for line in lines if line.startswith("data:image/png;base64,")), "")
    if not data_uri:
        raise RuntimeError(
            f"generate_imagedata returned no valid data URI for args={args}. "
...
def generate_breakout_layout(width, height):
    ...
    object_types.append(make_sprite_object("Paddle", 80, 16, 0, [
        ...
        {"behaviorId": "bound", "name": "BoundToLayout"}
    ]))
    ...
    game_instances.append(make_sprite_instance("Paddle", width // 2, height - 40, 80, 16, {
        ...
        "BoundToLayout": {"properties": {"bound-by": "edge"}}
    }))

    # Ball (set angle in world properties for initial direction)
    ball_inst = make_sprite_instance("Ball", width // 2, height - 80, 16, 16, {
        "Bullet": {"properties": {"speed": 0, "acceleration": 0, "gravity": 0, "bounce-off-solids": True, "set-angle": False, "enabled": True}}
    })
    ...
    ui_instances = [
        make_text_instance("ScoreText", 20, height - 30, 300, 24, "Score: 0   Lives: 3")
    ]
