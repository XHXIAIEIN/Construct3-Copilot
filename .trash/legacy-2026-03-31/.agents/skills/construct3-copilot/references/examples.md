...
Always paste the layout first, then the events, and finally run `.agents/skills/construct3-copilot/scripts/validate_output.py` on the
...
- **Usage**
  ...
  3. Run `python .agents/skills/construct3-copilot/scripts/validate_output.py tests/examples/breakout_events.json`.
...
- **Usage**
  ...
  3. Run `python .agents/skills/construct3-copilot/scripts/validate_output.py tests/examples/platformer_events.json`.
...
- **Key Event Snippets**
  ...
  // Player rotation toward mouse
  {
    ...
    "actions": [{"id": "set-angle", "objectClass": "Player", "parameters": {"angle": "angle(Player.X, Player.Y, Mouse.X, Mouse.Y)"}}]
  }

  // Fire bullet on click
  {
    "eventType": "block",
    "conditions": [{"id": "on-click", "objectClass": "Mouse", "parameters": {"mouse-button": "left", "click-type": "clicked"}}],
