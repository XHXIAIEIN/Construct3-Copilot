...
2. **Schema/RAG retrieval** – Use `.agents/skills/construct3-copilot/.agents/skills/construct3-copilot/scripts/query_schema.py` to confirm ACE IDs/params. If
   `data/project_analysis` exists, run `.agents/skills/construct3-copilot/scripts/query_examples.py` to pull real usage snippets for
   grounding.
...
7. **Validation & testing** – Execute `.agents/skills/construct3-copilot/.agents/skills/construct3-copilot/scripts/validate_output.py '<json>'`. Suggest at least one
...
- **ImageData generation** (`.agents/skills/construct3-copilot/scripts/generate_imagedata.py`)
  - `python .agents/skills/construct3-copilot/scripts/generate_imagedata.py --color red --width 32 --height 32`
  - `python .agents/skills/construct3-copilot/scripts/generate_imagedata.py --kenney player --color blue`
  - `python .agents/skills/construct3-copilot/scripts/generate_imagedata.py --file sprite.png`
- **Layout presets** (`.agents/skills/construct3-copilot/scripts/generate_layout.py`)
  - `python .agents/skills/construct3-copilot/scripts/generate_layout.py --preset platformer -W 640 -H 480 -o layout.json`
  - `python .agents/skills/construct3-copilot/scripts/generate_layout.py --preset breakout -W 640 -H 480 -o layout.json`
- **Schema lookup** (`.agents/skills/construct3-copilot/.agents/skills/construct3-copilot/scripts/query_schema.py`)
  - `python .agents/skills/construct3-copilot/.agents/skills/construct3-copilot/scripts/query_schema.py plugin sprite set-animation`
  - `python .agents/skills/construct3-copilot/.agents/skills/construct3-copilot/scripts/query_schema.py behavior platform simulate-control`
- **Example mining** (`.agents/skills/construct3-copilot/.agents/skills/construct3-copilot/scripts/query_examples.py`)
  - Use only if `data/project_analysis/*.json` exists.
  - `python .agents/skills/construct3-copilot/.agents/skills/construct3-copilot/scripts/query_examples.py action create-object`
  - `python .agents/skills/construct3-copilot/.agents/skills/construct3-copilot/scripts/query_examples.py top actions 20`
- **Validation** (`.agents/skills/construct3-copilot/.agents/skills/construct3-copilot/scripts/validate_output.py`)
  - `python .agents/skills/construct3-copilot/.agents/skills/construct3-copilot/scripts/validate_output.py '<json>'` (accepts stdin or file path).

## 6.1 Out-of-Scope Recovery Loop

When requests exceed reliable scope (missing prerequisites, unsupported export target, repeated validation failures), do not guess.

1. **Declare boundary** – State exactly which part is out of scope.
2. **Fallback deliverable** – Provide the smallest validated JSON subset the user can paste now.
3. **Targeted clarification** – Ask one blocking question to return to in-scope generation.
4. **Retry with evidence** – Re-run validator and include pass/fail evidence in the reply.

Use failure history before retrying:

- `python scripts/find_similar_failures.py "<error text>"`
- Update `tests/regressions/failure_cases.jsonl` after resolving a new failure pattern.
